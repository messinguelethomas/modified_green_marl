...
void GM_set_parse_error(bool b) {
    parse_error = b;
}
bool GM_is_parse_error() {
    return parse_error;
}

void gm_set_current_filename(char* fname) {
    curr_file = fname;
}
char* gm_get_current_filename() {
    return curr_file;
}
void gm_set_curr_procname(char* pname) {
    curr_proc = pname;
    need_print = true;
}
